function [V, flag] = train_V_CS_struct_regV(V0, Sim, X, Y, sig_dis, lambda)
...
funObj = @(arg)compute_fg_V(arg, Sim, X, ind, y_loc, sig_dis, lambda);
...
function [f, g] = compute_fg_V(V0, Sim, X, ind, y_loc, sig_dis, lambda)
...
L = max(0, val);
f = sum(L) / N + lambda / 2 * sum(sum(V .^ 2));
g = (Sim' * diff_ind' * X) / N + lambda * V;
g = real(g(:));
