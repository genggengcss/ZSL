function SynC_regV(task, dataset, opt, direct_test)

%% Input
% task: 'train', 'val', 'test'
% dataset: 'AWA', 'CUB', 'SUN'
% opt: opt.lambda: the regularizer coefficient on W in training (e.g, 2 .^ (-30 : -17). You may want to try even smaller ones if the performance improves)
        ...
        for j = 1 : nr_fold
            ...
            for k = 1 : length(opt.Sim_scale)
                ...
                if (strcmp(opt.loss_type, 'OVO'))
                    V = train_V_OVO_regV([], Sim_base, Xbase, Ybase, opt.lambda(i));
                elseif (strcmp(opt.loss_type, 'CS'))
                    V = train_V_CS_regV([], Sim_base, Xbase, Ybase, opt.lambda(i));
                elseif (strcmp(opt.loss_type, 'struct'))
                    V = train_V_struct_regV([], Sim_base, Xbase, Ybase, Sig_dist(unique(Ybase), unique(Ybase)), opt.lambda(i));
                else
            ...
            save(['../SynC_CV_classifiers/SynC_regV_' opt.loss_type '_classCV_' dataset '_split' num2str(opt.ind_split) '_googleNet_' norm_method '_' Sim_type...
...
if (strcmp(task, 'val'))
    ...
    for i = 1 : length(opt.lambda)
        load(['../SynC_CV_classifiers/SynC_regV_' opt.loss_type '_classCV_' dataset '_split' num2str(opt.ind_split) '_googleNet_' norm_method '_' Sim_type...
            '_lambda' num2str(opt.lambda(i)) '.mat'], 'V_record');
        for j = 1 : nr_fold
            Ybase = Ytr;
            Ybase(fold_loc{j}) = [];
    ...
    save(['../SynC_CV_results/SynC_regV_' opt.loss_type '_classCV_' dataset '_split' num2str(opt.ind_split) '_googleNet_' norm_method '_' Sim_type '.mat'], 'acc_val', 'opt');
end

%% testing
if (strcmp(task, 'test'))
    if(isempty(direct_test))
        load(['../SynC_CV_results/SynC_regV_' opt.loss_type '_classCV_' dataset '_split' num2str(opt.ind_split) '_googleNet_' norm_method '_' Sim_type '.mat'], 'acc_val', 'opt');
    ...
    if (strcmp(opt.loss_type, 'OVO'))
        V = train_V_OVO_regV([], Sim_tr, Xtr, Ytr, lambda);
    elseif (strcmp(opt.loss_type, 'CS'))
        V = train_V_CS_regV([], Sim_tr, Xtr, Ytr, lambda);
    elseif (strcmp(opt.loss_type, 'struct'))
        V = train_V_struct_regV([], Sim_tr, Xtr, Ytr, Sig_dist(unique(Ytr), unique(Ytr)), lambda);
    else
        display('Wrong loss type!');
        return;
    end
    ...
    save(['../SynC_results/SynC_regV_' opt.loss_type '_' dataset '_split' num2str(opt.ind_split) '_googleNet_' norm_method '_' Sim_type...
