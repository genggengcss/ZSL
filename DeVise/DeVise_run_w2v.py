...
def train(core_model,folder_dir_train,folder_dir_eval,loss_fn):
    ...
    pth=folder_dir_eval
    vec_m=[] ##seen matrix of training set
    ids=[]  # id of 19 seen classes
    for f in dirs_seen:#for seen class
        id = int(re.sub("\D", "", f))
        ids.append(id)
        idx = id - 1
    ...
    print("size of vec mat seen:", vec_m.shape) # 19,500
    vec_m=vec_m.transpose()

    best_acc_te_1 ,best_acc_te_2,best_acc_te_5  = 0, 0, 0
    writer = SummaryWriter('w2v')
    for epoch in range(epoch_num):
        ...
        #test
        #print('eval begin:')
        combine_dir = "./ZSL_DATA/test/combine"
        acc_test_1,acc_test_2,acc_test_5,loss_total_test=DeVise_test_GZSL_w2v.dtest(te,core_model,combine_dir,GPU,loss_fn)
        #print(
        ...
        if epoch>5 and (acc_test_1>best_acc_te_1 or acc_test_2>best_acc_te_2 or acc_test_5>best_acc_te_5):
            ...
            if acc_test_5>best_acc_te_5:
                ...
                ans += "%"
            # './save_model'
            torch.save(core_model.state_dict(),os.path.join(output_folder, 'w2v'+str(round(time.time())) + 'epoch' + str(epoch) +ans+ '.pkl'))

        # each 10 epoch save
        elif(epoch>0 and epoch % 10 ==0):
            torch.save(core_model.state_dict(), os.path.join(output_folder, 'w2v'+str(round(time.time()))+'epoch'+str(epoch) + '.pkl'))
...
if __name__ == "__main__":
    ...
    folder_dir_eval="./ZSL_DATA/test/unseen"
    loss_fn=nn.MSELoss()
