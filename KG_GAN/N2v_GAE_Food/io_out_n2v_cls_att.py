...
def prepare_n2v2():
    ...
    # n2v = np.zeros((len(wnids), (cls_seen_embed.shape[1] + att_seen_embed.shape[1])), dtype=np.float)
    n2v = np.zeros((len(wnids), cls_seen_embed.shape[1]), dtype=np.float)
    ...
    for i in range(len(wnids)):
        if wnids[i] in seen:
            ...
            cls_graph_index = cls_graph_nodes.index(wnids[i])
            cls_corresp = cls_seen_corresp.index(cls_graph_index)
            n2v[i] = cls_seen_embed[cls_corresp]
            ...
            n2v[i] = cls_unseen_embed[cls_corresp]
        else:
            continue
    # save wnids together
    wnids_cell = np.empty((len(wnids), 1), dtype=np.object)
...
def prepare_n2v3():
    ...
    # n2v = np.zeros((len(wnids), (cls_seen_embed.shape[1] + att_seen_embed.shape[1])), dtype=np.float)
    n2v = np.zeros((len(wnids), att_seen_embed.shape[1]), dtype=np.float)
    ...
    for i in range(len(wnids)):
        if wnids[i] in seen:
            ...
            n2v[i] = att_seen_embed[att_corresp]
        elif wnids[i] in unseen:
            att_graph_index = att_graph_nodes.index(wnids[i])
            att_corresp = att_unseen_corresp.index(att_graph_index)
            n2v[i] = att_unseen_embed[att_corresp]
...
'''
Exp2: cls:500.pkl, att:1850.pkl
Exp3: cls:.pkl, att:1900.pkl
Exp4: cls:300.pkl, att: 1850.pkl
Exp5: cls:400.pkl, att: 1000.pkl

Exp6: cls:800.pkl, att: 2900.pkl

Exp8: cls:450.pkl, att:1900.pkl
Exp9: cls:750.pkl (950.pkl), att: 2600.pkl

Exp10: cls: 60.pkl, 1000.pkl
'''

if __name__ == '__main__':
    seen, unseen = load_class()

    cls_seen_embed, cls_unseen_embed, cls_seen_corresp, cls_unseen_corresp = extract_embed('cls', '60.pkl')
    # att_seen_embed, att_unseen_embed, att_seen_corresp, att_unseen_corresp= extract_embed('att', '1000.pkl')
    save_file = 'g2v-cls.mat'

    # save_file = 'g2v-att.mat'
    # save_file = 'n2v-2000.mat'
