...
def load_data2(dataset):
    ...
    for key, value in graph.items():
        ...
        if edge:
            for ed in edge:
                edge_list.append((key, ed))
    print(len(edge_list))
    print(len(set(edge_list)))
...
def load_data_cls(graph_file, input_fea_file):
    ...
    matcontent = scio.loadmat(input_fea_file)
    features = matcontent['w2v']
...
def mask_test_edges2(adj):
    # Function to extract val set with 20% positive links
